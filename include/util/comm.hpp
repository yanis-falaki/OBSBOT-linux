...
template<typename T> inline T *getPtrHelper(T *ptr)
{
	return ptr;
}
 ...
 */
enum { DEV_ERROR = 100, DEV_WARN = 200, DEV_INFO = 300, DEV_DEBUG = 400 };

/**
 * @brief  The callback function definition for log output used by the device
 *         library, if no callback function is registered, it will be output to
 *         the console.
 ...
 */
typedef void (*dlog_handler_t)(int32_t lvl, const char *msg, va_list args,
			       void *p);
